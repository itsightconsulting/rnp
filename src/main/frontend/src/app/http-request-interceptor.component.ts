...
export class HttpRequestInterceptor implements HttpInterceptor{
    ...
    constructor(private readonly cookie: CookieService, private readonly onWakeUp: OnStartUpService){
    }

    interceptFirstApi(req: HttpRequest<any>, next: HttpHandler){
        const clonedRequest = req.clone({url: req.url.replace('/api/fi', this.rnpApiBsRoute),
                                                 headers: req.headers.set('Authorization', 'Bearer ' + this.cookie.get('rnp_api_token'))});
    ...
    intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        if(!req.url.startsWith('/api/fi/oauth')){
            if(req.url.startsWith('/api/fi')) {
                return this.interceptFirstApi(req, next);
            }
        }else{
            return this.interceptGetTokenApi(req, next);
        }
        return next.handle(req)
    }

    interceptGetTokenApi(req: HttpRequest<any>, next: HttpHandler){
        const clonedRequest = req.clone({ url: req.url.replace('/api/fi', this.rnpApiBsRoute)});
        return next.handle(clonedRequest).pipe(map((event: HttpEvent<any>) => {
            return event;
        }), catchError((error: HttpErrorResponse) => {
